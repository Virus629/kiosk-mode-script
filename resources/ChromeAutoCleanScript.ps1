...
do {
    ...
    $shellSite = "http://example.com/kiosk.php?c="+$($env:COMPUTERNAME)
    ...
    if (($hasRestarted -eq 0) -And ($idleTime -gt $kioskIdleTimeout)) { # If idleTime (in minutes) is greater than kioskIdleTimeout ==> restart chrome
        ...
        # Start-Sleep -Milliseconds 5000 # Wait 5s before starting new process
    ...
    Start-Sleep -Seconds 10 # Wait 10 secs so we dont burn cpu
} while ($true)
